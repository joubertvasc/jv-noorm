...
const BasicCrud_1 = require("../generate/basicCrud/BasicCrud");
// Example of extending the BasicCrud class
class BrandsCrudModule extends BasicCrud_1.BasicCrud {
...
// Example of extending the BasicCrud class
class ModelsCrudModule extends BasicCrud_1.BasicCrud {
    async findByName(name) {
        if (!this.db)
            throw new Error('Database connection is not defined');
        return this.db?.queryRow({
...
(async () => {
    ...
    await db.exec({
        command: env_1.env.DB_TYPE === dbType_1.DBType.MariaDB
    ...
    try {
        ...
        try {
            ...
            if (!fordBrand)
                throw new Error('Ford not found');
            await models.create({
                data: {
        ...
        if (!fiatBrand)
            throw new Error('Fiat brand not found');
