...
(async () => {
  ...
  try {
    try {
      ...
      await db.exec({
        command: `CREATE TABLE IF NOT EXISTS tmp_brands(id INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
      ...
      await db.exec({
        command: `CREATE TABLE IF NOT EXISTS tmp_models(id INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
      ...
      const brandInserted = await db.insert({
        command: `INSERT INTO tmp_brands(brand_name) VALUES (?)`,
        values: ['Ford'],
      });
      ...
      const modelInserted = await db.insert({
        command: `INSERT INTO tmp_models(brand_id, model_name) VALUES (?, ?)`,
      ...
      await db.delete({ command: 'DELETE FROM tmp_brands WHERE id = ? ', values: [1] });

      // Method 2
      db.setSoftDelete(false); // To revert previous set
      await db.delete({
        command: 'DELETE FROM tmp_models WHERE id = ?',
        values: [3],
        options: {
          softDelete: true,
        },
